---
title: "os x 10.10安装goagent"
date: 2015-03-26T00:00:00+0800
lastmod: 2015-03-26T00:00:00+0800
draft: false
keywords: ["os x 10.10","goagent","安装"]
tags:  ["os x 10.10","goagent","安装"]
description: "记录在os x10.10安装goagent的过程与问题"
categories: ["常用技巧"]
tags: ["os x 10.10","goagent","安装"]
---
...
(转载自:https://github.com/goagent/goagent/blob/wiki/InstallGuide.md)
...
没有运行上传命令，会出现连接的各种问题，如/_gh/路径找不到，或者去掉path后面的路径，又说head头太短问题。

在上传过程中最好用vpn连接能上传到google，上传成功后，只要将appid写到proxy.ini中就行了。

###问题二

打不开google等https连接的网站，主要是证书问题，直接从chrome中设置-->高级设置-->管理证书,导入CA.crt证书，如下图：

![ca证书]( /imgs/osx安装goagent/ca.png)

###问题三

在访问google的时候会出问题，访问不了，后面查明原因是不能由dns获的ip,简单的方法就是将dns直接改为8.8.8.8(前提是你的电脑能ping通这个ip,要不只能换其它的dns的ip了)

###问题四

在访问google.com.hk时有时会有问题，最好是直接访问google.com，不要加.hk后缀，或者可以直接访问google.com.tw
